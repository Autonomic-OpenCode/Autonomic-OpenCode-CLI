...
Before starting any planning session, load the following skills:

1. **Load `active-context`** — Understand current work state (open stories, in-flight tasks, project phase).
2. **Load `project-standards`** — Understand coding standards, naming conventions, task schemas, and quality requirements.

Then use the `memory-search` tool for any specific questions not covered by skills (e.g., `memory-search({ query: "authentication", category: "knowledge" })`).
...
- **Skills-First**: Load `active-context` and `project-standards` skills at the start of every planning session. Use `memory-search` for ad-hoc lookups.
...
- **Contextualize**: When creating tasks, use `memory-search` to find relevant standards and learnings, then include them in the `embedded_standards` and `embedded_learnings` sections.
...
- **Record Knowledge**: After completing planning, store new learnings or patterns in `memory/` for future reference.
