...
3.  **Implement**: Use the `create_file` tool to generate the agent definition file in `.opencode/agents/[agent-name].md`.
...
- Create the agent definition file in `.opencode/agents/[agent-name].md`.
- **Configuration**:
...
- **Skills-First**: Load relevant skills and use the `memory-search` tool to check for existing agent patterns before creating new ones.
